...
exports.createColor = async (req, res) => {
    ...
    const color = await Color.create({ colorName });
      ...
      res.status(500).json({ error: 'An error occurred while creating the color' });
    }
  };
...
exports.getAllColors = async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'An error occurred while fetching the colors' });
      }
};
...
exports.getColorById = async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'An error occurred while fetching the Color' });
      }
}

exports.updateColor = async (req, res) => {
    try {
        const [updated] = await Color.update(req.body, { where: { id: req.params.id } });
        if (updated) {
